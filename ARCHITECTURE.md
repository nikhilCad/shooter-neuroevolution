...
- **speciation**: genomes are grouped into species by `GeneticDistance` (excess/disjoint gene counts + average weight difference, counting only *enabled* connections — a disabled gene has zero effect on `Activate`, so it shouldn't count toward genome size or a mismatch; earlier including disabled genes let crossover's gene bloat crush excess/disjoint toward zero once raw gene-list size passed the normalization cutoff, collapsing the whole population to one species for an entire run — confirmed across two separate sweeps before the fix). Each species mostly reproduces within itself, which is what protects a brand-new structural mutation long enough to have its weights tuned before it has to compete head-to-head with a fully-optimized genome from a different lineage. A species' champion is carried over unchanged once it has enough members; one all-time-best genome is preserved regardless of what happens to its species.
